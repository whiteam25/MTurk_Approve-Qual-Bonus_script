...
#Remaining issues
# 1. Write some code that automatically grabs the results csv file from MTurk 
...
previous_record_id = 0 #set this value equal to the record ID of the last submitted record in redcap for study (TAC starts at 1, TMHP starts at 5000)
who_paid = 0  #Change value to correspond to your initial: MS = 0, GW = 1, RLS = 2 MU = 3, CH = 4, CC = 5, AB = 6 
which_study = 0 #Change the value to correspond to the correct study: TAC = 1, morgan no ami = 2, morgan ami = 3 
bonus_level = 0 #0: $0.00, 1: $2.00, 2: $3.45
bonus_amount = 0
bonus_amount_str = "***"
...
payment_tracking_complete = 2 #This sets the value of for "tracking" to complete. No need to change. 
non_completer_qual = 1 #This designates that everyone got the appropriate qualification to not repeat the study. No need to change. 
notes = ""
redcap_upload_file = 'REDCAP_Tracking_Upload.csv'
...
#determine what observations are a new assignment submission (important for writing the redcap csv file) and avoiding errors with AWS fxns
workers_to_add = []
total_num_workers = []
...
#that only NEW submission within a batch will be approved or rejected (prevents client call from calling an error) 
workers_to_approve = []
	...
	if row["Answer.screenercode"] == screener_code or row["Answer.screenercode"] in full_surv_completion_codes:
		if run_mode == "pay" and response_assign_status["Assignment"]["AssignmentStatus"] == "Submitted":
			approve_response = client.approve_assignment(
				AssignmentId = row["AssignmentId"],
				RequesterFeedback ='TAC Survey HIT has been approved (worth $0.05). If you submitted the full survey completion code, you will be recieving a bonus payment as well.',
			)
...
#This loop will loop through each observation in the .csv, assign num_results to hold the values for number of previous bonus payment on assignment
...
#Calculate estimated cost if program is run in the "pay" mode (includes cost to worker and mturk fee)
total_cost = 0
total_approval_cost = 0 
total_bonus_cost = 0
if len(total_num_workers) > 9: 
	total_approval_cost = (len(test_workers_to_approve) * 0.05) * 1.4
else: 
	total_approval_cost = (len(test_workers_to_approve) * 0.05) * 1.2
total_bonus_cost = (len(test_workers_to_pay) * bonus_amount) * 1.2
total_cost = total_approval_cost + total_bonus_cost
...
if run_mode == "test":
	...
	print("If you run this with the 'pay' flag, you'd pay ${} bonuses to a TOTAL of {} workers for this batch ({} new bonuses would be paid)".format(bonus_amount,len(workers_to_pay), len(test_workers_to_pay)))
	...
	print("If you run this with the 'pay' flag, this will charge our account ~${:0.2f} (${:0.2f} in approvals, ${:0.2f} in bonuses).".format(total_cost, total_approval_cost,total_bonus_cost))
	print("\n")
...
if run_mode == "pay":
	...
	print("This program has paid a ${} bonus to the following {} new workers on MTurk (a total of {} bonuses have been paid for this batch).".format(bonus_amount, new_bonus, len(workers_to_pay)))
	print("See below for full details on which new workers have received bonuses.")
	for i in test_workers_to_pay:
		print(i)
	print("\n")
	print("We currently have ${} remaining in our account (this number may not reflect the costs from approving HITs (${:0.2f}) - should represent account balance after bonus payments (${:0.2f})).".format(client.get_account_balance()["AvailableBalance"], total_approval_cost, total_bonus_cost))
	print("\n")
